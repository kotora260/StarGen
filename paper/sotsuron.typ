...
#show: master_thesis.with(
  ...
  abstract_ja: [
      ...
      以上の解析結果から，シミュレーションにおける分子雲進化の構造について，自己重力による収縮が平衡に近づき，内部では小規模な構造がより大きい構造へ成長するという，より詳細なシナリオ推定することが可能になった．
  ]
)
...
//この章では得られた解析結果のサイズ，質量，ビリアルパラメータの3つの推移をもとに，分子雲進化を考察していく．
...
トランクのサイズや質量の変化についてより詳細に考察するため，ビリアルパラメータ$alpha_("Gvir")$を@eq:vir を用いて算出した．なお$R$はトランクの面積から，トランクを円形と考えることで導出した．また$sigma$はトランク内部の速度スペクトルを足し合わせて最小二乗法を用いてガウスフィッテイングを行い，フィッティング結果の半値幅とした．@img:dv_trunk_2D に速度分散$sigma$の変化を示す． //@img:fwhm_400 から@img:fwhm_2800 にそれぞれの時点でのフィッテングの結果を示す．
...
サイズ，質量，ビリアルパラメータの平均値の変化を，それぞれ@img:inner_size，@img:inner_mass，@img:inner_vir に示す．
...
サイズ，質量，ビリアルパラメータの平均値の変化を，それぞれに@img:size_ave，@img:mass_ave，@img:vir_ave に示す．青の線がリーフ，オレンジの線がブランチの平均値変化を表している．なおサイズについては，視線速度，位置，位置の三次元空間における体積であることを考慮し$"pc"^2$ #unit("km/s")という単位を用いている．

これらのグラフから，リーフのサイズ，質量，ビリアルパラメータの平均値はすべて増減を繰り返していることがわかる．一方ブランチのサイズ，質量の平均値は時間発展に伴い徐々に増加していることがわかる．またブランチのビリアルパラメータについても，特に1.1 Myrから2.2 Myrにかけて増加していることがわかる．
...
三次元散布図の解析で得られた各構造についても，サイズと質量のヒストグラムを作成した．結果を@img:size_hist_3D_，@img:mass_hist_3D に示す．青のビンがリーフの個数，オレンジのビンがブランチの個数を表している．
