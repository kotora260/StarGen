...
例えばMiura et al. (2012)では, 近傍銀河M33のCO観測によって71個の巨大分子雲(GMC)を同定し, それらを進化段階に応じて4つのカテゴリーに分類している. ただしこの進化段階のカテゴリーはGMCと, M33内の若い星の集団(YSG), H II領域の, 観測時点での位置関係によって解釈されたものである. 例として, この4つのカテゴリーのうち, 大規模な星形成が行われる前の状態であるType Aカテゴリーに属するGMC-60を@img:gmc_typea に, 現在大規模な星形成が行われている状態であるType Cカテゴリーに属するGMC-1を@img:gmc_typec に示す. GMC-60と比べ, GMC-1はより階層的なCO積分強度図を持っていることがわかる. 
...
#img(
 ...
 このように, 観測を用いた研究では複数の分子雲を比較してその進化段階を相対的に評価することとなる. 反対に, 単一の分子雲の観測データのみからでは, 構造進化を理解するのは困難である. 分子雲が収縮し星となるまでに要する時間はは$10^5$年から$10^6$年であり, 観測データはその長い時間における一時点の情報に過ぎないからである.
...
一方, 流体の中で分子雲コアに対応するシンク粒子などの構造がどのような過程を経て形成されるかについては, まだあまり考えられていなかった. 前小節で述べたように, 観測データのみを用いて単一の分子雲の構造進化を追うことは難しい. しかし, 観測データを十分に再現したシミュレーションであれば, 自己重力流体の時間発展を解析することで, このような構造進化の研究を容易に行うことができる.
