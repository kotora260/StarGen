...
#show: master_thesis.with(
  ...
  abstract_ja: [
      ...
      解析の結果, 積分強度図において同定された構造のうち, 最も外側の構造については自己重力により収縮していく様子が確認できた. しかしその収縮スピードやこの構造のビリアルパラメータの推移から, 分子雲全体としてビリアル平衡に近づいていることがわかった. また内部構造については, 膨張と収縮を繰り返していることがわかった. 三次元散布図の解析結果からは, 同定された各構造が膨張と収縮を繰り返していることがわかった. 一方で時間発展につれ比較的大きな構造が生じ, 成長していく様子も見られた. 
...
シミュレーションを用いて研究する場合, そのシミュレーションが観測データを再現するものであることが重要である.
...
前節で述べたように, 観測データを用いて分子雲の構造進化を追うことは難しい. しかし, 観測データを十分に再現したシミュレーションであれば, 自己重力流体の時間発展を解析することで, このような構造進化の研究を容易に行うことができる.
...
本研究では, 先述のシミュレーションのうち, それぞれ$0.5702$ $"Myr"$, $1.104$ $"Myr"$, $1.703$ $"Myr"$, $2.188$ $"Myr"$の時点における質量のデータ(それぞれ m400, m1000, m2200, m2800 とラベリングしている)を福島助教に提供していただき, 解析した. これらのデータは観測データとの比較が容易な, 視線速度, 位置, 位置からなる三次元データである. $40$ × $65$ × $65$個のボクセルの中に質量が数値として格納されている. また1つのボクセルは #qty(0.65,"km/s") × $1.4$ $"pc"$ × $1.4$ $"pc"$ の三次元空間に相当する.
...
以下はトランクの質量変化のグラフである. グラフから, トランクの質量は徐々に減少していることがわかる. この事実は, @img:dendro_400_contour から@img:dendro_2800_contour で見られるような, トランクの内部におけるフィラメント構造の発達を示している. フィラメント構造の発達により質量の粗密が増大することで, Dendrogram の最小値を上回るような質量を持つ部分が減少するため, 質量の数値に減少傾向がみられたと考えられる.
...
なおサイズについては, 視線速度 × 位置 × 位置の三次元空間における体積であることを考慮し$"pc"^2$ #unit("km/s")という単位を用いている.
...
@img:size_hist_3D_ より, 積分強度図上の構造と同様, 三次元散布図上の各構造においてもサイズの増加と減少が繰り返されていると考えられる. 一方でm2200やm2800において,$1000$ $"pc"^2$ #unit("km/s")を上回るサイズのリーフやブランチが生じていることがわかる. 特にm2800における最大のブランチがリーフを覆うような巨大な構造となっていることが,図から定性的にも確認できる.
...
まず分子雲は, 全体としては自己重力によって徐々に収縮する一方, 内部でフィラメント構造を発展させる. また分子雲内部に存在する小規模な構造は, そのサイズと質量について増加と減少を繰り返す. その後, 分子雲全体としてビリアル平衡に近づくことによって, 自己重力による収縮やフィラメント構造の発展のスピードは減少していく. 一方で内部の小規模な構造は, 合体や散逸といった原因によって, より大きな構造を形成していく.
