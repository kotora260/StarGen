...
#import "@preview/metro:0.1.0": * // solar mass:$M_⊙$, pc:$"pc"$, Myr:$"Myr"$
...
シミュレーションは磁場のない一様な分子雲を想定して雲質量$10^6$ $M_⊙$,面密度$350$ $M_⊙$ $"pc"^(-2)$,半径$30$ $"pc"$の一様な流体の球を自己重力により変化させた.

== データ詳細

本研究では,先述のシミュレーションのうち,それぞれ$0.5702$ $"Myr"$,$1.104$ $"Myr"$,$1.703$ $"Myr"$,$2.188$ $"Myr"$の時点における質量のデータ(それぞれm400,m1000,m2200,m2800とラベリングしている)を福島助教に提供していただき,解析した.これらのデータは観測データとの比較が容易な,視線速度,位置,位置からなる三次元データである.$40$×$65$×$65$個のボクセルの中に質量が数値として格納されている.また1つのボクセルは#qty(0.65,"km/s")×$1.4$ $"pc"$×$1.4$ $"pc"$の三次元空間に相当する.
...
以下はトランクのサイズ変化のグラフである.グラフから,トランクは徐々に収縮していることがわかる.しかしグラフの傾きに相当する収縮のスピードは減少していることがわかる.
...
以下はトランクの質量変化のグラフである.グラフから,トランクの質量は徐々に減少していることがわかる.この事実は,@img:dendro_400_contour から@img:dendro_2800_contour で見られるような,トランクの内部におけるフィラメント構造の発達を示している.フィラメント構造の発達により質量の粗密が増大することで,Dendrogramの最小値を上回るような質量を持つ部分が減少するため,質量の数値に減少傾向がみられたと考えられる.
...
トランクのサイズや質量の変化についてより詳細に考察するため,ビリアルパラメータ$alpha_("Gvir")$を@eq:vir を用いて算出した.なお$R$はトランクの面積から,トランクを円形と考えることで導出した.また$sigma$はトランク内部の速度スペクトルを足し合わせて最小二乗法を用いてガウスフィッテイングを行い,フィッティング結果の半値幅とした.以下にそれぞれの時点でのフィッテングの結果を示す.
