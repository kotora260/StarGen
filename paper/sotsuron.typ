...
以下では分子雲の全体像についてその構造進化を考えていく．そのために，積分強度図の解析で得られた構造のうち，最外部の構造であるトランクのサイズ，質量，ビリアルパラメータの変化を考察していく．
...
#img(
  ...
  caption: [トランクのビリアルパラメータ変化],
) <img:alpha_gvir_trunk_2D>

== 積分強度図上の内部構造の変化

以下では分子雲の内部構造についてその構造進化を考えていく．そのために，積分強度図の解析で得られた構造のうち，トランク以外の構造のサイズ，質量、ビリアルパラメータの平均値を算出してその変化を考察していく．

サイズ，質量，ビリアルパラメータの平均値の変化を，それぞれ@img:inner_size，@img:inner_mass，@img:inner_vir に示す．これらのグラフから，内部構造のサイズ，質量，ビリアルパラメータの平均値はすべて増減を繰り返していることがわかる．

#img(
  image("c:media/inner_size.png", width: 70%),
  caption: [内部構造のサイズの平均値変化],
) <img:inner_size>

#img(
  image("c:media/inner_mass.png", width: 70%),
  caption: [内部構造の質量の平均値変化],
) <img:inner_mass>

#img(
  image("c:media/inner_vir.png", width: 70%),
  caption: [内部構造のビリアルパラメータの平均値変化],
) <img:inner_vir>
