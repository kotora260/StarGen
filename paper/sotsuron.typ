...
本研究では,先述のシミュレーションのうち,それぞれ$0.5702$ $"Myr"$, $1.104$ $"Myr"$, $1.703$ $"Myr"$, $2.188$ $"Myr"$の時点における質量のデータ(それぞれ m400, m1000, m2200, m2800 とラベリングしている)を取得して解析した. これらのデータは視線速度, 位置, 位置からなる三次元データである. $40$ × $65$ × $65$ 個のボクセルの中に質量が数値として格納されている. また1つのボクセルは #qty(0.65,"km/s") × $1.4$ $"pc"$ × $1.4$ $"pc"$ の三次元空間に相当する.
...
以下では積分強度図の解析で得られた構造のうち, 最外部の構造であるトランクのサイズと質量の変化を考察していく.
